...
async function drawProductCard(doc, x, y, w, h, product, imageBuffers, uploadsRoot, settings) {
  ...
  // Nombre del producto: ocupa todo el ancho de la tarjeta arriba, para que
  // los nombres largos no se corten con "...".
  const nameW = w - pad * 2 - (waLink ? hintSize + 6 : 0);
  doc
    .fillColor(TEXT_DARK)
    .font("Helvetica-Bold")
    .fontSize(11)
    .text(product.name, x + pad, y + pad, { width: nameW, height: 28 });

  // Aviso de "consultar por WhatsApp" — toda la tarjeta es clicable (ver doc.link arriba)
  if (waLink) {
    iconChat(doc, x + w - hintSize - pad, y + pad, hintSize, BRAND_COLOR);
  }

  const contentY = y + pad + 28 + 6;
  const imgSize = y + h - contentY - pad;
  const imgX = x + pad;
  const imgY = contentY;
