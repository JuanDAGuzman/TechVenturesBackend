...
async function drawProductCard(doc, x, y, w, h, product, imageBuffers, uploadsRoot, settings) {
  ...
  const hintSize = 11;
  // Si la tarjeta tiene el icono de "consultar por WhatsApp" en la esquina
  // superior derecha, dejamos espacio para que el nombre no se le encime.
  const nameW = waLink ? textW - hintSize - 6 : textW;

  doc
    .fillColor(TEXT_DARK)
    .font("Helvetica-Bold")
    .fontSize(11)
    .text(product.name, textX, y + 14, { width: nameW, height: 32, ellipsis: true });
