...
function drawCoverPage(doc, products, categories, settings) {
  ...
  doc.fillColor("#ffffff").text(badgeText, badgeX, badgeY + 7, { width: badgeW, align: "center" });
  ...
  rows.forEach((r) => {
    ...
    r.forEach((p) => {
      ...
      doc.fillColor("#ffffff").text(p.name.toUpperCase(), px, py + 6, { width: p.w, align: "center" });
      px += p.w + pillGap;
    });
    py += pillH + pillGap;
  });
  ...
  const hintText = "Toca el producto que te interese y te escribimos por WhatsApp";
  doc.font("Helvetica-Bold").fontSize(10);
  ...
  // Sitio web — el dato más llamativo
  doc.link(iconX, rowY - 4, linkW, 26, "https://techventuresco.com");
  iconGlobe(doc, iconX, rowY - 2, iconSize, BRAND_COLOR);
  doc
    .fillColor(BRAND_COLOR)
    .font("Helvetica-Bold")
    .fontSize(15)
    .text("techventuresco.com", textX, rowY, { width: textW, underline: true });
  rowY += 30;

  if (settings.whatsapp_number) {
    const waDigits = settings.whatsapp_number.replace(/\D/g, "");
    doc.link(iconX, rowY - 3, linkW, 24, `https://wa.me/${waDigits}`);
    iconChat(doc, iconX, rowY - 1, iconSize, BRAND_COLOR);
    doc
      .fillColor(BRAND_COLOR)
      .font("Helvetica-Bold")
      .fontSize(12)
      .text(`WhatsApp: ${formatWhatsappDisplay(settings.whatsapp_number)}`, textX, rowY + 1, {
        width: textW,
        underline: true,
      });
    rowY += 27;
  ...
  if (settings.instagram_handle) {
    ...
    doc.link(iconX, rowY - 3, linkW, 24, `https://instagram.com/${igHandle}`);
    iconInstagram(doc, iconX, rowY - 1, iconSize, BRAND_COLOR);
    doc
      .fillColor(BRAND_COLOR)
      .font("Helvetica-Bold")
      .fontSize(12)
      .text(`Instagram: ${settings.instagram_handle}`, textX, rowY + 1, {
        width: textW,
        underline: true,
      });
    rowY += 27;
...
function drawCategoryHeader(doc, name, color, count, contentWidth) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(13)
    .text(name.toUpperCase(), textX, y + 10, { lineBreak: false });
...
async function drawProductCard(doc, x, y, w, h, product, imageBuffers, uploadsRoot, settings) {
  ...
  // Aviso de "consultar por WhatsApp" — toda la tarjeta es clicable (ver doc.link arriba)
  if (waLink) {
    const hintSize = 11;
    iconChat(doc, x + w - hintSize - 10, y + 10, hintSize, BRAND_COLOR);
  }
